...
def index():
    ...
    # Gabungkan data API dengan data internal dari DB
    enriched_countries = []
...
def country_detail(iso3):
    ...
    if not detail:
        # Fallback jika API bermasalah tetapi data DB ada
        db_country = get_country(iso3)
        detail = {
            "name": db_country['name'],
            "official_name": db_country['name'],
            "flag": f"https://flagcdn.com/w320/{iso3.lower()}.png",
            "population": 0,
            "region": "Global",
            "subregion": "Global",
            "capital": "N/A",
            "currency": f"N/A ({db_country['code']})",
    ...
    # Ambil data ekonomi tambahan secara paralel
...
def admin_add():
    if request.method == 'POST':
        ...
        # Hubs, Challenges, Universities split by comma
        ...
        # Validasi Data Input
        if not re.match("^[A-Z]{3}$", cca3):
            flash("Kode ISO3 Negara harus terdiri dari tepat 3 huruf kapital.", "danger")
        ...
        # Simpan ke Database
        success = insert_country(cca3, name, maturity, code, desc, full_description, hubs, challenges, universities, sectors)
        if success:
            cache.clear() # Hapus cache agar data baru langsung muncul
            flash(f"Profil teknologi untuk negara {name} berhasil ditambahkan!", "success")
            return redirect(url_for('index'))
        else:
            flash(f"Negara dengan kode ISO3 '{cca3}' sudah ada di database.", "danger")
            return render_template('admin_add.html')
